  ...
  return (
    <div className="min-h-screen flex flex-col bg-hero bg-grain relative" style={{ color: 'var(--cream)' }}>
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-20 relative z-10">
        ...
        <p className="text-center text-base max-w-xl mx-auto mb-16" style={{ color: 'rgba(246,247,249,0.65)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="card-dark p-8 flex flex-col items-center text-center">
            ...
            <div className="text-sm mb-8" style={{ color: 'rgba(246,247,249,0.55)' }}>2 proposals every month</div>
          ...
          <div className="card-dark p-8 flex flex-col items-center text-center">
            ...
            <div className="text-sm mb-8" style={{ color: 'rgba(246,247,249,0.55)' }}>pay only when you send one</div>
            <Link href="/dashboard" className="btn-outline w-full py-3 text-xs tracking-[0.2em] uppercase" style={{ borderColor: 'var(--gold)', color: 'var(--cream)' }}>
              Buy One
            </Link>
          </div>
          <div className="card-dark card-featured p-8 flex flex-col items-center text-center relative">
            ...
            <div className="text-sm mb-8" style={{ color: 'rgba(246,247,249,0.55)' }}>unlimited proposals</div>
        ...
        <div className="mb-20">
          ...
          <p className="text-center text-sm mb-10 max-w-xl mx-auto" style={{ color: 'rgba(246,247,249,0.55)' }}>
          ...
          <div className="max-w-lg mx-auto flex flex-col gap-3">
            {COMPARISON.map((c) => (
              <div
                ...
                style={
                  c.highlight
                    ? { backgroundColor: 'rgba(30,79,216,0.1)', border: '1px solid var(--gold)' }
                    : { border: '1px solid var(--gold-dim)' }
                }
              >
                <div style={{ color: c.highlight ? 'var(--gold-light)' : 'rgba(246,247,249,0.75)', fontWeight: c.highlight ? 600 : 400 }}>
                  {c.name}
                </div>
                <div className="text-right">
                  <div style={{ color: c.highlight ? 'var(--cream)' : 'rgba(246,247,249,0.75)', fontWeight: c.highlight ? 600 : 400 }}>
                    {c.price}
                  </div>
                  <div className="text-xs" style={{ color: 'rgba(246,247,249,0.45)' }}>{c.note}</div>
                </div>
              </div>
            ))}
          </div>
          <p className="text-center text-xs mt-6 max-w-lg mx-auto" style={{ color: 'rgba(246,247,249,0.4)' }}>
          ...
          <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--gold-dim)' }}>
                <th className="text-left py-4 font-normal" style={{ color: 'rgba(246,247,249,0.5)' }}>Feature</th>
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.label} style={{ borderBottom: '1px solid var(--gold-dim)' }}>
                  <td className="py-4" style={{ color: 'rgba(246,247,249,0.7)' }}>{row.label}</td>
                  <td className="py-4 text-center" style={{ color: 'rgba(246,247,249,0.6)' }}>{row.free}</td>
                  <td className="py-4 text-center" style={{ color: 'rgba(246,247,249,0.6)' }}>{row.single}</td>
        ...
        <p className="text-center text-xs tracking-[0.15em] uppercase mb-16" style={{ color: 'rgba(246,247,249,0.4)' }}>
