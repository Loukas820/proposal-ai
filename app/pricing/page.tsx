  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--cream)', color: 'var(--charcoal)' }}>
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-20">
        ...
        <p className="text-center text-base max-w-xl mx-auto mb-16" style={{ color: 'rgba(27,30,38,0.65)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="card p-8 flex flex-col items-center text-center">
            <div className="text-sm tracking-[0.2em] uppercase mb-4" style={{ color: 'var(--gold)', fontWeight: 600 }}>Free</div>
            <div className="text-4xl mb-2" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--navy)' }}>$0</div>
            <div className="text-sm mb-8" style={{ color: 'rgba(27,30,38,0.55)' }}>2 proposals every month</div>
          ...
          <div className="card card-featured p-8 flex flex-col items-center text-center relative">
            <div className="absolute -top-3 px-4 py-1 text-xs tracking-[0.2em] uppercase" style={{ backgroundColor: 'var(--gold)', color: '#ffffff', fontWeight: 600 }}>
              Best Value
            </div>
        ...
        <div className="mb-20">
          <h2 className="text-2xl text-center mb-3" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--navy)' }}>
            Priced To Undercut, Not Just Compete
          </h2>
          <p className="text-center text-sm mb-10 max-w-xl mx-auto" style={{ color: 'rgba(27,30,38,0.55)' }}>
          ...
          <div className="max-w-lg mx-auto flex flex-col gap-3">
            {COMPARISON.map((c) => (
              <div
                ...
                style={
                  c.highlight
                    ? { backgroundColor: 'rgba(30,79,216,0.05)', border: '1px solid var(--gold)' }
                    : { border: '1px solid var(--hairline)' }
                }
              >
                <div style={{ color: c.highlight ? 'var(--navy)' : 'rgba(27,30,38,0.75)', fontWeight: c.highlight ? 600 : 400 }}>
                  {c.name}
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs" style={{ color: 'rgba(27,30,38,0.45)' }}>{c.note}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.label} style={{ borderBottom: '1px solid var(--hairline)' }}>
                  <td className="py-4" style={{ color: 'rgba(27,30,38,0.7)' }}>{row.label}</td>
                  <td className="py-4 text-center" style={{ color: 'rgba(27,30,38,0.6)' }}>{row.free}</td>
                  <td className="py-4 text-center" style={{ color: 'rgba(27,30,38,0.6)' }}>{row.single}</td>
                  <td className="py-4 text-center" style={{ color: 'var(--navy)', fontWeight: 600 }}>{row.unlimited}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mb-20">
          <h2 className="text-2xl text-center mb-10" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--navy)' }}>
            Pricing Questions
          </h2>
          <div className="flex flex-col gap-6 max-w-2xl mx-auto">
            {MINI_FAQ.map((item) => (
              <details key={item.q} className="group" style={{ borderBottom: '1px solid var(--hairline)', paddingBottom: '1.5rem' }}>
                <summary className="cursor-pointer text-base list-none flex items-center justify-between" style={{ fontFamily: 'var(--font-serif)', fontWeight: 600, color: 'var(--navy)' }}>
                  {item.q}
        ...
        <p className="text-center text-xs tracking-[0.15em] uppercase mb-16" style={{ color: 'rgba(27,30,38,0.4)' }}>
